...
function showQuestion() {
    ...
    if (currentQuestionIndex >= quizQuestions.length) {
        console.error(
            'Försöker visa en fråga utanför quizQuestions-arrayens gränser.'
        )
        return
    }
...
function checkAnswer() {
    const userCorrectAnswerDiv = document.querySelector(
        '#user-correct-answer'
    ) as HTMLElement // Hämta det dynamiska elementet
    const userWrongAnswerDiv = document.querySelector(
        '#user-wrong-answer'
    ) as HTMLElement // Hämta det dynamiska elementet
    ...
    if (currentQuestionIndex >= quizQuestions.length) {
        console.error(
            'currentQuestionIndex är utanför gränsen för quizQuestions-arrayen.'
        )
        return
    }
...
function restartQuiz() {
    if (run == quizQuestions.length / 10) {
        //om man har spelat genom frågorna återställs allt för en ny spelomgång
...
function timerStart() {
    //sätter tiderna till 0 varje gång man callar på funktionen för att återställa siffrorna
    counter = 0
    second = 0
    minute = 0
